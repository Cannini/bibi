...
async def update_room(room_id: str, request: UpdateRoomRequest) -> dict[str, Any]:
    # Chama a regra do jogo que editamos no Passo 2
    room = await game_service.update_room(room_id, request)
    
    # O aviso de websocket em tempo real já estava aqui.
    # Nós só adicionamos o "playerId" no final para o front-end saber de quem veio.
    await emit_room_event("room.updated", room, {"playerId": request.player_id})
